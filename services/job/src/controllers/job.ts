...
export const createCompany = TryCatch(async(req: AuthenticatedRequest, res)=>{
  ...
  const existingCompany = await sql`SELECT company_id FROM companies WHERE name = ${name}` as any[];
...
export const deleteCompany = TryCatch(async(req: AuthenticatedRequest, res) => {
  ...
  if (!company) {
    throw new ErrorHandler(
      404, 
      "Company not found or you're not authorized to delete the company"
    );
  }
...
export const getCompanyDetails = TryCatch(
  async(req:AuthenticatedRequest, res)=> {
    ...
    const [companyData] = await sql`
      ...
      FROM companies c WHERE c.company_id = ${id} GROUP BY c.company_id;` as any;
