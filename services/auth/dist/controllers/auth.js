...
export const registerUser = TryCatch(async (req, res, next) => {
    ...
    `;
        ...
        let axiosData;
        try {
            const { data } = await axios.post(`${process.env.UPLOAD_SERVICE}/api/utils/uploads`, { buffer: filebuffer.content });
            axiosData = data;
        }
        catch (error) {
    ...
    if (registerdUser) {
        const token = jwt.sign({ id: registerdUser?.user_id }, process.env.JWT_SEC, { expiresIn: '15d' });
        res.json({
...
export const forgetPassword = TryCatch(async (req, res, next) => {
  ...
  `;
    ...
    publicToTopic("send-mail", message).catch((error) => {
        console.error("Failed to send the message to kafka with error: ", error);
    });
    res.json({
        message: "If that email exists, we have sent a reset link",
    });
